...
let server, wss;
const java = vscode.workspace
  .getConfiguration("thread-dump-analyzer")
  .get("jdkPath")
  ? vscode.workspace
      .getConfiguration("jarExplorer")
      .get("jdkPath")
      .endsWith(".exe")
    ? vscode.workspace.getConfiguration("jarExplorer").get("jdkPath")
    : vscode.workspace.getConfiguration("jarExplorer").get("jdkPath") + ".exe"
  : "java";
...
function activate(context) {
  const disposable = vscode.commands.registerCommand(
    "thread-dump-analyzer.viewThreadDump",
    async (uri) => {
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          "Failed to analyze thread dump: " + error.message
        );
        return;
      }
      ...
      html = html.replace(/(href|src)="(.+?)"/g, (match, p1, p2) => {
        const resourcePath = vscode.Uri.file(
          path.join(context.extensionPath, "media", "dist", p2)
        );
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      wss.on("connection", (ws) => {
        ...
        ws.send(
          JSON.stringify({ type: "welcome", payload: "Hello from VS Code!" })
        );
